...
export function createDashboardTools(accessibleProjectIds: string[] | null) {
  ...
  return {
    getProjectOverview: tool({
      description:
        "Lấy tổng quan tất cả dự án: tên, lead count, F1, booking, CPL.",
    ...
    getCampaignAnalysis: tool({
      description:
        "Phân tích chi tiết campaigns của 1 dự án: tên campaign, F1, đang chăm, tổng lead.",
      inputSchema: z.object({
    ...
    getLeadBreakdownByStage: tool({
      description: "Phân tích lead theo stage của 1 dự án.",
      inputSchema: z.object({
        projectName: z.string().describe("Tên dự án"),
      }),
      execute: async ({ projectName }) => {
    ...
    getEmployeePerformance: tool({
      description: "Xem hiệu quả nhân viên sale trong 1 dự án.",
      inputSchema: z.object({
        projectName: z.string().describe("Tên dự án"),
      }),
      execute: async ({ projectName }) => {
    ...
    getCostAnalysis: tool({
      description: "Phân tích chi phí marketing: tổng chi phí, CPL, CP/F1.",
      inputSchema: z.object({
        projectName: z.string().describe("Tên dự án"),
      }),
      execute: async ({ projectName }) => {
        ...
        const cost = costResult[0]!.totalCost;
